...
def _NYSE_CAL_HOLIDAYS(start, end):
    """Regular NYSE/Nasdaq market holidays between two dates (used to tell a real
    missing session apart from a normal closure when backfilling from FRED)."""
    ...
    return set(_Cal().holidays(start, end))
...
SP_STALE_DAYS = 5      # a live S&P feed should be within a week of the last session

def _sp_is_fresh(s, label):
    """A long-but-stale series is worse than no series: sp_bench() as-of-clamps both
    sides of its ratio to the same final value and silently returns 0.0 for every
    date past the end, which flatlines the S&P benchmark on the page. Stooq's ^SPX
    went stale on 2026-09-01 and still returned ~25k rows, so a row-count check
    passed it straight through and the yfinance fallback never ran."""
    if s is None or not len(s):
        return False
    lag = int(np.busday_count(s.index.max().date(), pd.Timestamp.today().date()))
    if lag > SP_STALE_DAYS:
        log(f"  daily S&P from {label} is STALE (last {s.index.max().date()}, "
            f"{lag} business days behind) -- rejecting, trying next source")
        return False
    return True
...
def daily_sp():
    ...
    try:
        ...
        if "Close" in df.columns and len(df) > 500:
            ...
            out = _clean_series(df.set_index("Date")["Close"])
            if _sp_is_fresh(out, "Stooq"):
                log(f"  daily S&P from Stooq: {len(df)} rows (through {out.index.max().date()})")
                return out
    except Exception as e:
    ...
    try:
        ...
        if isinstance(close, pd.DataFrame):
            close = close.iloc[:, 0]
        out = _clean_series(close)
        if _sp_is_fresh(out, "Yahoo"):
            log(f"  daily S&P from Yahoo: {len(out)} rows (through {out.index.max().date()})")
            return out
        log(f"  daily S&P from Yahoo: {len(out)} rows -- STALE but best available, using it")
        return out
    except Exception as e:
...
try:
    ndx = daily_ndx()
    # --- GAP GUARD: protect the published Nasdaq-100 history from source gaps ----------
    # Yahoo occasionally returns ^NDX MISSING a settled session (2026-07-29 and
    # 2026-08-07 both vanished on the 2026-08-24 fetch). Rebuilding timeline_qqq from a
    # gapped series understates 20-day realized vol and spuriously RAISES the live
    # leverage target -- the 1.05x <-> 1.31x flip that had the account reversing trades.
    # Fix: merge the fresh fetch onto the PREVIOUSLY PUBLISHED series, append-only. A
    # short fetch can then only ADD days, never drop settled ones; only the last
    # RESETTLE_NDX bars are re-settled from the fresh feed (provisional close -> settled).
    RESETTLE_NDX = 2
    try:
        _prev = (json.load(open("data.json", encoding="utf-8")).get("timeline_qqq")
                 if os.path.exists("data.json") else None)
        if _prev and _prev.get("dates") and _prev.get("px"):
            prev_ndx = pd.Series([float(p) for p in _prev["px"]],
                                 index=pd.to_datetime(_prev["dates"]))
            prev_ndx = prev_ndx[~prev_ndx.index.duplicated(keep="last")].sort_index()
            if len(prev_ndx) > RESETTLE_NDX:
                if len(ndx) == 0:
                    # source totally failed this run -- fall back to the published series so
                    # the QQQ timeline is not dropped; the freshness gate below still flags
                    # the staleness and refuses to publish, keeping the last good data.json.
                    ndx = _clean_series(prev_ndx)
                    log("  ndx gap-guard: fresh fetch EMPTY -- restored full published series "
                        "(freshness gate will flag staleness)")
                else:
                    cut = prev_ndx.index[-RESETTLE_NDX]          # re-settle window boundary
                    fresh_dates = set(ndx.index)
                    merged = {pd.Timestamp(k): float(v) for k, v in ndx.items()}  # fresh base + warmup
                    restored = []
                    for k, v in prev_ndx.items():
                        if k < cut:                              # settled history: published is authoritative
                            if k not in fresh_dates: restored.append(k)   # a day the fresh fetch dropped
                            merged[pd.Timestamp(k)] = float(v)   # freeze it / restore it
                        # k >= cut: leave the fresh value (re-settled tail + brand-new days)
                    ndx = _clean_series(pd.Series(merged))
                    if restored:
                        log(f"  ndx gap-guard: RESTORED {len(restored)} settled session(s) the fresh "
                            f"fetch dropped: {[k.strftime('%Y-%m-%d') for k in sorted(restored)[-6:]]}")
                    log(f"  ndx gap-guard: append-only merge onto {len(prev_ndx)} published bars "
                        f"(re-settled last {RESETTLE_NDX}); series now {len(ndx)} rows")
    except Exception as _e:
        log(f"  ndx gap-guard skipped: {_e}")

    # --- SECONDARY SOURCE: backfill sessions Yahoo dropped, from FRED NASDAQ100 -----------
    # The gap-guard above can only restore days that exist in the PUBLISHED series. A session
    # missing from BOTH Yahoo and history (e.g. 2026-08-28) stayed a hole -- which understated
    # 20-day realized vol and delayed a real leverage step by a day. FRED series NASDAQ100 is
    # the daily NDX close published by Nasdaq, Inc. itself, in the SAME index units, so a
    # missing bar drops straight in with no scaling. FRED posts late in the evening, so it is
    # NOT used for today's bar (Yahoo stays primary) -- only to fill PRIOR missing sessions.
    # Fail-safe: any problem here leaves `ndx` exactly as it was.
    try:
        if FRED_KEY and len(ndx) > 250:
            _end = ndx.index.max()
            _start = max(ndx.index.min(), _end - pd.Timedelta(days=180))
            _wk = pd.bdate_range(_start, _end)
            _hol = set(_NYSE_CAL_HOLIDAYS(_wk[0], _wk[-1]))
            _want = [x for x in _wk if x not in _hol and x not in set(ndx.index) and x < _end]
            if _want:
                log(f"  ndx secondary source: {len(_want)} session(s) still missing after gap-guard "
                    f"{[x.strftime('%Y-%m-%d') for x in _want[-6:]]} -- querying FRED NASDAQ100")
                _fr = _clean_series(fred("NASDAQ100"))
                _filled, _rejected = [], []
                for _d in _want:
                    if _d not in _fr.index:
                        continue
                    _v = float(_fr.loc[_d])
                    # sanity: the filled bar must sit within +/-10% of the nearest known close,
                    # so a bad or mis-scaled quote can never inject a fake volatile session.
                    _prior = ndx[ndx.index < _d]
                    _ref = float(_prior.iloc[-1]) if len(_prior) else _v
                    if _ref > 0 and abs(_v / _ref - 1.0) > 0.10:
                        _rejected.append((_d, _v, _ref)); continue
                    ndx.loc[_d] = _v; _filled.append(_d)
                if _filled:
                    ndx = _clean_series(ndx)
                    log(f"  ndx secondary source: FILLED {len(_filled)} session(s) from FRED "
                        f"{[x.strftime('%Y-%m-%d') for x in _filled]} (Nasdaq, Inc. via FRED)")
                for _d, _v, _ref in _rejected:
                    log(f"  ndx secondary source: REJECTED FRED {_d:%Y-%m-%d} value {_v:,.1f} "
                        f"(>10% from prior close {_ref:,.1f}) -- left missing on purpose")
                if not _filled and not _rejected:
                    log("  ndx secondary source: FRED had none of the missing sessions either")
    except Exception as _e:
        log(f"  ndx secondary source skipped (non-fatal): {_e}")
...
# ---------- COMPLETENESS GATE: no missing trading session in the recent window ----------
# realized_vol (and therefore the vol-scaled leverage target) is only as trustworthy as a
# gap-free price series. The gap guard above prevents most holes by restoring dropped days;
# this is the backstop. If the published Nasdaq-100 timeline is STILL missing an expected
# NYSE session in the last ~120 days, refuse to write -- turn a silent vol-corrupting hole
# into a loud red run, exactly like the freshness gate turns a stale feed into one.
if timeline_qqq and timeline_qqq.get("dates"):
    try:
        from pandas.tseries.holiday import (
            AbstractHolidayCalendar, Holiday, nearest_workday,
            USMartinLutherKingJr, USPresidentsDay, GoodFriday,
        ...
        class _NYSECal(AbstractHolidayCalendar):
            rules = [
                ...
                USLaborDay, USThanksgivingDay,
                Holiday("Christmas", month=12, day=25, observance=nearest_workday)]
        _qd = pd.to_datetime(timeline_qqq["dates"]).sort_values()
        _end = _qd[-1]
        _start = max(_qd[0], _end - pd.Timedelta(days=120))
        _wk = pd.bdate_range(_start, _end)                      # Mon-Fri
        _hol = set(_NYSECal().holidays(_wk[0], _wk[-1]))
        _expected = [x for x in _wk if x not in _hol]           # regular NYSE/Nasdaq sessions
        _have = set(_qd)
        _missing = [x.strftime("%Y-%m-%d") for x in _expected if x not in _have]
        if _missing:
            _nmiss = len(_missing); _MAX_STALE = 3
            log(f"  COMPLETENESS GATE: {_nmiss} expected trading session(s) missing from the "
                f"Nasdaq-100 timeline in the last 120 days: {_missing}")
            if os.environ.get("DAILY_VT", "0") == "1":
                log("  (DAILY_VT export run -- proceeding; a recent hole is immaterial to a 40-yr study.)")
            elif _nmiss <= _MAX_STALE:
                # The append-only gap-guard already restored every session present in the published
                # feed, so anything still missing here is UNRESTORABLE (absent from Yahoo AND history)
                # -- re-running won't fix it. A handful of such holes shift 20-day vol by a bounded
                # amount and self-heal as they age out of the window (~20 sessions), so we WRITE with a
                # loud warning rather than freeze the pipeline. A larger cluster is a real feed break.
                log(f"  {_nmiss} UNRESTORABLE session(s) (<= {_MAX_STALE}): Yahoo lacks them and the "
                    f"gap-guard can't backfill, so a re-run won't help. Writing anyway -- bounded vol "
                    f"impact, self-heals in ~20 sessions. (Add a second NDX source to remove these.)")
            else:
                log(f"  {_nmiss} missing (> {_MAX_STALE}) -- refusing to write (exit 1). Likely a feed "
                    f"breakdown, not an isolated hole. Re-run once the feed is complete.")
                sys.exit(1)
        log(f"  completeness gate OK: {len(_expected)} expected sessions in the last 120 days, "
            f"all present (through {_end.date()})")
    except SystemExit:
        raise
    except Exception as _e:
        log(f"  completeness gate skipped (non-fatal): {_e}")
...
if os.environ.get("DAILY_VT","0") == "1":
  try:
    ...
    # --- EXPORT for the seal-on-the-1st study: daily nowcast + monthly-gauge intermediates ---
    # Writes nowcast_export.json only. Lets us seal the daily nowcast on the 1st and re-run
    # v3.3 head-to-head against the monthly gauge, and see exactly why the gauge diverges.
    try:
        capef_m = epct(d["cape"]); volsup_m = 1 - epct(d["rvol"]); comp_m = 1 - epct(d["spread"])
        _col = lambda s: [None if pd.isna(x) else round(float(x), 4) for x in s]
        monthly = {"month":  [pd.Timestamp(p).strftime("%Y-%m") for p in d.index],
                   "cape":   _col(d["cape"]),  "rvol": _col(d["rvol"]), "spread": _col(d["spread"]),
                   "capef":  _col(capef_m),    "volsup": _col(volsup_m), "comp": _col(comp_m),
                   "V":      _col(d["V"]),      "T": _col(d["T"])}
        _vda = Vd_a.dropna(); _tda = Td_a.reindex(_vda.index); _spx = spx.reindex(_vda.index)
        daily = {"date": [x.strftime("%Y-%m-%d") for x in _vda.index], "V": _col(_vda), "T": _col(_tda),
                 "sp_px": [None if pd.isna(x) else round(float(x), 2) for x in _spx]}
        qqq = ({"date": list(timeline_qqq["dates"]), "px": list(timeline_qqq["px"])}
               if timeline_qqq and timeline_qqq.get("dates") else None)
        with open("nowcast_export.json", "w", encoding="utf-8") as f:
            json.dump({"note": "seal-on-1st study: S&P-aligned daily nowcast + monthly gauge; QQQ vehicle history included",
                       "monthly_gauge": monthly, "daily_nowcast": daily, "daily_qqq": qqq}, f)
        log(f"  wrote nowcast_export.json  (monthly {len(monthly['month'])} rows; "
            f"daily nowcast {len(daily['date'])} rows"
            + (f" {daily['date'][0]}..{daily['date'][-1]}" if daily['date'] else "")
            + (f"; QQQ {len(qqq['date'])} rows" if qqq else "; QQQ n/a") + ")")
    except Exception as _ex:
        log(f"  nowcast_export skipped: {_ex}")
  except Exception as e:
    import traceback
