...
if timeline_qqq and timeline_qqq.get("dates"):
    try:
        ...
        if _missing:
            log(f"  COMPLETENESS GATE FAILED: {len(_missing)} expected trading session(s) missing "
                f"from the Nasdaq-100 timeline in the last 120 days: {_missing}")
            if os.environ.get("DAILY_VT", "0") == "1":
                log("  (DAILY_VT export run: proceeding anyway so the nowcast study can run -- a recent "
                    "single-day hole is immaterial to a 40-year backtest. Production runs still block.)")
            else:
                log("  Refusing to write a gapped data.json (exit 1) -- a hole here corrupts realized "
                    "vol and flips the leverage target. Re-run once the feed is complete.")
                sys.exit(1)
